...
def fetch_user_and_role(active_role_dict, session) -> Users:
    username = active_role_dict["user"]
    db_user = session.exec(select(Users).where(Users.username == username)).one()
    role = active_role_dict["active_role"]
    return (db_user, role)
